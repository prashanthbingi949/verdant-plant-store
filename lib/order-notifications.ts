...
function buildTrackingUrl(order: NotificationOrder) {
  if (!order.order_id || !order.payment_id) return null;
  return `${getSiteUrl()}/order-tracking?order=${encodeURIComponent(order.order_id)}&payment=${encodeURIComponent(order.payment_id)}`;
}
...
async function sendEmail(order: NotificationOrder, copy: { subject: string; title: string; message: string }) {
  ...
  const status = copy.subject.includes("confirmed") ? "Paid" : copy.subject.includes("on the way") ? "Shipped" : "Delivered";
  const trackingButton = trackingUrl
    ? `<a href="${escapeHtml(trackingUrl)}" style="display:inline-block;margin-top:20px;padding:12px 18px;border-radius:999px;background:#ddf27a;color:#101510;text-decoration:none;font-weight:700">Track your order</a>`
    : "";
