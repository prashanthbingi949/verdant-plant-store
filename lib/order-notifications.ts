...
export async function sendOrderNotifications(order: NotificationOrder, status: "shipped" | "delivered") {
  ...
  if (order.email && process.env.RESEND_API_KEY && process.env.RESEND_FROM_EMAIL) {
