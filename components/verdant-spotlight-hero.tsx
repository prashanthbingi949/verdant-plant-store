...
export default function VerdantSpotlightHero() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [homeResponse, siteResponse, authResponse] = await Promise.all([fetch("/api/cms/home", { cache: "no-store" }), fetch("/api/cms/site", { cache: "no-store" }), fetch("/api/auth/me", { cache: "no-store" })]);
        const home = await homeResponse.json().catch(() => null); const site = await siteResponse.json().catch(() => null); const auth = await authResponse.json().catch(() => null);
  ...
  const accountClass = scrolled ? "rounded-full border-2 border-[#202d20] bg-[#f4f5e9] px-4 py-2 text-sm font-bold text-[#202d20] hover:bg-[#e9eadc] hover:text-[#202d20] transition-colors" : "rounded-full border-2 border-white/50 bg-black/55 px-4 py-2 text-sm font-bold text-white backdrop-blur-md hover:bg-black/55 hover:text-white transition-colors";
  ...
  return <section ref={heroRef} className="verdant-spotlight relative w-full overflow-hidden bg-black h-screen" style={{ height: "100dvh", minHeight: "560px", fontFamily: "Inter, Arial, sans-serif" }}>
    ...
    <div className="absolute bottom-6 sm:bottom-12 left-5 right-5 sm:left-auto sm:right-10 md:right-14 z-50 max-w-full sm:max-w-[320px] flex flex-col items-center text-center gap-3 sm:gap-4 hero-anim hero-fade" style={{ animationDelay: "0.75s" }}>
      <p className="max-w-[300px] text-center text-xs sm:text-[13px] leading-relaxed text-white/78 italic" style={{ fontFamily: "'Brush Script MT', 'Segoe Script', 'Snell Roundhand', cursive" }}>{hero.description}</p>
      <Link href={hero.primaryHref} className="bg-[#ddf27a] text-[#101510] hover:bg-[#c9e55f] text-sm font-medium px-7 py-3 rounded-full transition-all hover:scale-[1.03] active:scale-95 hover:shadow-lg hover:shadow-lime-300/20 inline-flex items-center gap-2 pointer-events-auto">{hero.primaryLabel} <ArrowIcon /></Link>
    </div>
  </section>;
}
