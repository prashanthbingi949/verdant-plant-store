    ...
    <>
      {open && (
        <button
          ...
          className="fixed inset-0 z-[998] cursor-default bg-[#101510]/35 backdrop-blur-[2px]"
        />
      )}

      <aside
        aria-hidden={!open}
        className={`fixed right-0 top-0 z-[999] flex h-dvh w-full max-w-[420px] flex-col bg-[#f4f5e9] opacity-100 shadow-[-20px_0_70px_rgba(16,21,16,.22)] isolation-auto transition-transform duration-500 ease-[cubic-bezier(.2,.7,.2,1)] ${open ? "translate-x-0" : "pointer-events-none translate-x-full"}`}
      >
        <div className="flex items-center justify-between border-b border-black/10 bg-[#f4f5e9] px-6 py-5 sm:px-7">
          <div>
            <p className="text-[9px] font-black tracking-[.2em] text-[#52634b]">YOUR GREEN CORNER</p>
            <h2 className="mt-1 text-2xl font-semibold tracking-[-.035em] text-[#101510]">Cart <span className="text-sm font-semibold text-black/45">({itemCount})</span></h2>
          </div>
          <button type="button" onClick={() => setOpen(false)} aria-label="Close cart" className="grid h-10 w-10 place-items-center rounded-full border border-black/10 bg-white/55 text-xl text-[#202d20] transition hover:-translate-y-0.5 hover:bg-[#ddf27a]">×</button>
        </div>

        <div className="flex-1 overflow-y-auto bg-[#f4f5e9] px-6 py-6 sm:px-7">
          {items.length ? (
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="group flex gap-4 rounded-[22px] border border-black/8 bg-white/55 p-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3">
                      <div className="inline-flex items-center rounded-full border border-black/10 bg-[#f4f5e9] p-1">
                        <button type="button" onClick={() => updateQuantity(item.id, item.quantity - 1)} className="grid h-7 w-7 place-items-center rounded-full text-sm text-[#202d20] transition hover:bg-white">−</button>
                        <span className="w-7 text-center text-[11px] font-bold text-[#202d20]">{item.quantity}</span>
                        <button type="button" onClick={() => updateQuantity(item.id, item.quantity + 1)} className="grid h-7 w-7 place-items-center rounded-full text-sm text-[#202d20] transition hover:bg-white">+</button>
                      </div>
                      <strong className="text-sm text-[#202d20]">₹{(item.price * item.quantity).toLocaleString("en-IN")}</strong>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="flex h-full min-h-[300px] flex-col items-center justify-center text-center">
              ...
              <h3 className="mt-5 text-xl font-semibold text-[#101510]">Your cart is growing.</h3>
