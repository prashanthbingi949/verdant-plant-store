...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.error || "Unable to send a reset link.");
      ...
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col">
        ...
        <section className="mx-auto flex w-full max-w-5xl flex-1 items-center justify-center py-14">
          <div className="w-full max-w-xl rounded-[32px] border-2 border-[#202d20]/15 bg-[#fafbf4] p-6 shadow-[0_18px_60px_rgba(32,45,32,0.12)] sm:p-8">
            ...
            <h1 className="mt-4 text-5xl font-black leading-[0.9] tracking-[-0.06em] sm:text-6xl">Forgot your <span className="font-serif font-normal italic text-[#426a3f]">password?</span></h1>
            <p className="mt-5 text-sm leading-7 text-[#596654]">Enter your account email and we’ll send you a secure reset link.</p>

            <form onSubmit={handleSubmit} className="mt-7 space-y-4">
              <div>
                <label htmlFor="email" className="text-xs font-bold text-[#263926]">Email</label>
                <input id="email" name="email" type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 h-12 w-full rounded-2xl border-2 border-[#202d20]/15 bg-[#eef2df] px-4 font-medium text-[#101510] outline-none focus:border-[#202d20]" />
              </div>
              ...
              <button type="submit" disabled={loading} className="flex h-12 w-full items-center justify-center rounded-full bg-[#202d20] text-sm font-black text-[#f4f5e9] transition hover:bg-[#101510] disabled:cursor-not-allowed disabled:opacity-60">{loading ? "Sending…" : "Send reset link"}</button>
            </form>

            <div className="mt-6 flex justify-between gap-4 text-sm font-semibold text-[#263926]">
              <Link href="/login" className="underline decoration-2 underline-offset-4">Back to log in</Link>
              <Link href="/signup" className="underline decoration-2 underline-offset-4">Create an account</Link>
            </div>
          </div>
        </section>
