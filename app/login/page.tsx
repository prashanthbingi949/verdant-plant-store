    ...
    <main className="min-h-screen bg-[#eef2df] px-5 py-6 text-[#101510] sm:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col">
        <header className="flex items-center justify-between border-b-2 border-[#202d20]/15 pb-5">
          ...
          <div className="flex items-center gap-2 sm:gap-3"><Link href="/signup" className="rounded-full border-2 border-[#202d20] px-4 py-2 text-sm font-bold text-[#202d20]">Sign up</Link><Link href="/shop" className="rounded-full bg-[#202d20] px-4 py-2 text-sm font-bold !text-[#f4f5e9] transition hover:bg-[#101510]">Shop plants</Link></div>
        </header>
        ...
        <section className="mx-auto grid w-full max-w-5xl flex-1 items-center gap-10 py-14 lg:grid-cols-[1fr_.82fr] lg:gap-16">
          ...
          <div className="rounded-[32px] border-2 border-[#202d20]/15 bg-[#fafbf4] p-6 shadow-[0_18px_60px_rgba(32,45,32,0.12)] sm:p-8"><form onSubmit={handleSubmit} className="space-y-4"><div><label htmlFor="email" className="text-xs font-bold text-[#263926]">Email</label><input id="email" name="email" type="email" required autoComplete="email" className="mt-2 h-12 w-full rounded-2xl border-2 border-[#202d20]/15 bg-[#eef2df] px-4 font-medium text-[#101510] outline-none focus:border-[#202d20]" /></div><div><div className="flex items-center justify-between gap-4"><label htmlFor="password" className="text-xs font-bold text-[#263926]">Password</label><Link href="/forgot-password" className="text-xs font-bold text-[#315233] underline decoration-2 underline-offset-4 hover:text-[#101510]">Forgot password?</Link></div><input id="password" name="password" type="password" required autoComplete="current-password" className="mt-2 h-12 w-full rounded-2xl border-2 border-[#202d20]/15 bg-[#eef2df] px-4 font-medium text-[#101510] outline-none focus:border-[#202d20]" /></div>{error && <p role="alert" className="rounded-2xl border-2 border-red-900/15 bg-red-50 px-4 py-3 text-sm font-semibold text-red-900">{error}</p>}<button type="submit" disabled={loading} className="flex h-12 w-full items-center justify-center rounded-full bg-[#202d20] !text-[#f4f5e9] text-sm font-black transition hover:bg-[#101510] disabled:cursor-not-allowed disabled:opacity-60">{loading ? "Logging in…" : "Log in"}</button></form></div>
        </section>
