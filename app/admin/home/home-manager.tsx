...
export default function HomeManager() {
  ...
  function updateItem(key: string, field: string, index: number, value: unknown) {
    const section = sections.find((s) => s.section_key === key); if (!section) return;
    const items = Array.isArray(section.content[field]) ? [...section.content[field]] : [];
    items[index] = { ...items[index], ...(typeof value === "object" && value !== null ? value : {}) };
    updateContent(key, field, items);
  }

  async function save(section: Section) {
    setSaving(section.section_key); setNotice(""); setError("");
    const response = await fetch("/api/admin/home", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ section_key: section.section_key, content: section.content, active: section.active, sort_order: section.sort_order }) });
    const data = await response.json().catch(() => null);
    if (!response.ok) setError(data?.error || `Unable to save ${labels[section.section_key] || section.section_key}.`);
    else setNotice(`${labels[section.section_key] || section.section_key} saved.`);
    setSaving("");
  }
  ...
  function addCareItem() {
    ...
    const items = Array.isArray(section.content.items) ? section.content.items : [];
    updateContent("care", "items", [...items, { number: String(items.length + 1).padStart(2, "0"), title: "New care guide", href: "#care" }]);
  }
