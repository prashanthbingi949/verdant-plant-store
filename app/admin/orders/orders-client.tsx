...
export default function AdminOrdersClient({ orders }: { orders: Order[] }) {
  ...
  return (
    <section className="mt-10">
      <div className="flex flex-col gap-3 rounded-3xl border border-black/10 bg-white/55 p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex rounded-2xl bg-black/[.045] p-1">
          {filters.map((value) => (
            <button key={value} onClick={() => setFilter(value)} className={`rounded-xl px-4 py-2 text-xs font-bold capitalize transition ${filter === value ? "bg-[#202d20] text-[#f4f5e9]" : "text-black/50 hover:text-black"}`}>
              {value}
            </button>
          ))}
        </div>
      ...
      <div className="mt-5 space-y-3">
        {filteredOrders.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-black/15 bg-white/40 p-10 text-center">
            ...
            <p className="mt-2 text-sm text-black/45">Try a different search or filter.</p>
          </div>
        ) : (
                ...
                <button onClick={() => setOpenId(expanded ? null : order.id)} className="grid w-full gap-4 p-5 text-left sm:grid-cols-[1fr_auto_auto] sm:items-center">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[.08em] ${paid ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{paid ? "Paid" : "Pending"}</span>
                    </div>
                ...
                {expanded && (
                  <div className="border-t border-black/10 px-5 pb-5 pt-5">
                    <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
                      ...
                      <div>
                        <p className="text-[10px] font-bold tracking-[.16em] text-black/40">PAYMENT</p>
                        <div className="mt-3 space-y-2 text-sm text-black/65">
                          <p><span className="text-black/40">Status:</span> <strong className="text-black">{order.payment_status || "unknown"}</strong></p>
