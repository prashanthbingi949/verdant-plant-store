        ...
        return <article key={product.slug} className="overflow-hidden rounded-3xl border border-black/10 bg-white/55">
          ...
          {expanded && <div className="border-t border-black/10 p-5 sm:p-7">
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <label className="flex h-full min-h-12 items-center gap-3 rounded-2xl border border-black/10 bg-[#f4f5e9] px-4 text-sm"><input type="checkbox" checked={product.active} onChange={(e) => update(product.slug, { active: e.target.checked })} /> Show this product in the store</label>
            </div>
            <label className="mt-5 block text-sm">Description<textarea value={product.description} onChange={(e) => update(product.slug, { description: e.target.value })} rows={5} className="mt-2 min-h-32 w-full resize-y rounded-2xl border border-black/10 bg-[#f4f5e9] px-4 py-3 leading-6 outline-none focus:border-black/25" /></label>
