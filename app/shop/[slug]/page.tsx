...
export default function ProductPage({ params }: { params: { slug: string } }) {
  const product = catalog[params.slug as keyof typeof catalog];
  ...
  if (!product) {
    return (
      <main className="min-h-screen bg-[#f4f5e9] px-5 py-24 text-[#101510] sm:px-8">
        <div className="mx-auto max-w-xl text-center">
          <p className="text-[10px] font-bold tracking-[.2em] text-black/50">VERDANT SHOP</p>
          <h1 className="mt-4 text-5xl font-semibold tracking-[-.05em]">Plant not found.</h1>
          <Link href="/shop" className="mt-8 inline-flex rounded-full bg-[#202d20] px-6 py-3 text-sm font-bold text-[#f4f5e9]">Back to shop</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#f4f5e9] text-[#101510]">
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-10 sm:px-8 sm:py-14 lg:grid-cols-[1.04fr_.96fr] lg:gap-20 lg:py-20">
        <div className="overflow-hidden rounded-[32px] bg-[#dde6cf] shadow-sm">
          <PlantRender tone={product.tone} large />
        </div>
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <div className="flex h-12 w-fit items-center rounded-full border border-black/12 bg-white/45 p-1">
              <button onClick={() => setQuantity((value) => Math.max(1, value - 1))} className="h-10 w-10 rounded-full text-lg">−</button>
              <span className="w-8 text-center text-sm font-bold">{quantity}</span>
              <button onClick={() => setQuantity((value) => value + 1)} className="h-10 w-10 rounded-full text-lg">+</button>
            </div>
            <button onClick={() => setAdded(true)} className="flex h-12 flex-1 items-center justify-center rounded-full bg-[#202d20] px-6 text-sm font-bold text-[#f4f5e9] transition hover:translate-y-[-1px]">
              {added ? `Added · ${total.toLocaleString("en-IN")} total` : "Add to bag"}
            </button>
          </div>
