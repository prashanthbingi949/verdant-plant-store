...
const products = [
  { id: "monstera-deliciosa", name: "Monstera Deliciosa", category: "Indoor", level: "Easy", price: 1899, tone: "moss" as const, description: "A lush statement plant with large sculptural leaves." },
  { id: "snake-plant", name: "Snake Plant", category: "Indoor", level: "Easy", price: 899, tone: "sage" as const, description: "Architectural, resilient and happy in lower light." },
  { id: "jade-plant", name: "Jade Plant", category: "Succulents", level: "Easy", price: 649, tone: "lime" as const, description: "A compact succulent for desks, shelves and sunny corners." },
  { id: "bird-of-paradise", name: "Bird of Paradise", category: "Indoor", level: "Medium", price: 2499, tone: "moss" as const, description: "Bold tropical foliage for a room that needs presence." },
  { id: "string-of-pearls", name: "String of Pearls", category: "Succulents", level: "Medium", price: 1199, tone: "sage" as const, description: "Trailing beads that soften shelves and hanging planters." },
  { id: "lavender", name: "Lavender", category: "Outdoor", level: "Medium", price: 799, tone: "lime" as const, description: "Fragrant flowering stems made for bright balconies." },
  { id: "fiddle-leaf-fig", name: "Fiddle Leaf Fig", category: "Indoor", level: "Medium", price: 2199, tone: "moss" as const, description: "Large fiddle-shaped leaves and a polished silhouette." },
  { id: "aloe-vera", name: "Aloe Vera", category: "Succulents", level: "Easy", price: 699, tone: "sage" as const, description: "A sunny, low-maintenance classic with practical charm." },
];
...
function PlantIllustration({ tone }: { tone: string }) {
  ...
  return (
    <svg viewBox="0 0 360 320" className="h-full w-full" aria-hidden="true">
      <defs><radialGradient id={`glow-${tone}`} cx="50%" cy="38%" r="65%"><stop offset="0" stopColor="#f0f6c5" stopOpacity=".45" /><stop offset="1" stopColor="#f0f6c5" stopOpacity="0" /></radialGradient></defs>
...
function Pill({ active, children, onClick }: { active: boolean; children: React.ReactNode; onClick: () => void }) {
  return <button onClick={onClick} className={`rounded-full px-4 py-2 text-sm transition ${active ? "bg-[#202d20] text-[#f4f5e9]" : "bg-black/5 text-[#202d20] hover:bg-black/10"}`}>{children}</button>;
}
...
export default function ShopPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return [...result].sort((a, b) => sort === "low" ? a.price - b.price : sort === "high" ? b.price - a.price : a.id.localeCompare(b.id));
    ...
    <main className="min-h-screen bg-[#f4f5e9] text-[#101510]">
      <header className="sticky top-0 z-40 border-b border-black/10 bg-[#f4f5e9]/90 px-5 py-4 backdrop-blur-xl sm:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-5">
          ...
          <Link href="/cart" className="rounded-full bg-[#ddf27a] px-4 py-2 text-sm font-bold">Bag ({itemCount})</Link>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-8 pt-16 sm:px-8 sm:pt-20">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div><h1 className="max-w-3xl text-5xl font-semibold leading-[0.92] tracking-[-0.05em] sm:text-7xl">Plants for <span className="font-serif italic font-normal">slower</span> spaces.</h1><p className="mt-5 max-w-2xl text-sm leading-7 text-black/60 sm:text-base">Browse indoor plants, outdoor growers and easy-care favourites.</p></div>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-6 sm:px-8"><div className="flex flex-col gap-4 rounded-3xl border border-black/10 bg-white/35 p-4 sm:p-5 lg:flex-row lg:items-center">
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-5 pb-24 sm:grid-cols-2 sm:px-8 xl:grid-cols-4">
        {filtered.map((product) => (
          <article key={product.id} className="group">
            <Link href={`/shop/${product.id}`} className="block"><div className="relative aspect-[.82] overflow-hidden rounded-[28px] bg-[#e2e8d3] p-2 transition duration-500 group-hover:-translate-y-1 group-hover:shadow-xl">
            ...
            <div className="flex items-start justify-between gap-4 px-1 pt-4"><div><p className="text-[10px] tracking-wide text-black/45">{product.category} · {product.level}</p><Link href={`/shop/${product.id}`}><h2 className="mt-1 text-lg font-semibold tracking-[-0.03em] hover:underline">{product.name}</h2></Link><p className="mt-2 text-sm leading-6 text-black/55">{product.description}</p></div><strong className="shrink-0 text-base">₹{product.price.toLocaleString("en-IN")}</strong></div>
            <Link href={`/shop/${product.id}`} className="mt-4 flex h-11 w-full items-center justify-center rounded-full bg-[#202d20] text-sm font-bold text-[#f4f5e9] transition hover:scale-[1.01]">View plant</Link>
          </article>
        ))}
      </section>
