...
type Product = {
  id: string; slug: string; name: string; category: string; subcategory?: string; product_type?: "Plants" | "Gardening Supplies";
  level: string; price: number; size: string; description: string; tone: Tone; stock: number; active: boolean; featured?: boolean;
  sort_order?: number; image_url?: string | null; image_urls?: string[];
};
type Category = { id: string; name: string; slug: string; product_type: "Plants" | "Gardening Supplies"; description: string; sort_order: number; active: boolean };
type Subcategory = { id: string; category_id: string; name: string; slug: string; description: string; sort_order: number; active: boolean };
...
const palette: Record<Tone, { leaf: string; soft: string }> = {
  moss: { leaf: "#5f7e4a", soft: "#aebf96" }, sage: { leaf: "#6f8d63", soft: "#b9c6a5" }, lime: { leaf: "#8ea64d", soft: "#c8d66d" },
};
...
function Pill({ active, children, onClick }: { active: boolean; children: ReactNode; onClick: () => void }) { return <button type="button" onClick={onClick} className={`rounded-full px-4 py-2.5 text-sm font-semibold transition ${active ? "bg-[#202d20] text-[#f4f5e9] shadow-sm" : "bg-black/[.045] text-[#202d20] hover:bg-black/[.08]"}`}>{children}</button>; }
function HeartIcon({ filled = false }: { filled?: boolean }) { return <svg viewBox="0 0 24 24" className="h-5 w-5" fill={filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20.8 8.8c0 5.1-8.8 10.2-8.8 10.2S3.2 13.9 3.2 8.8A4.8 4.8 0 0 1 12 6.1a4.8 4.8 0 0 1 8.8 2.7Z" /></svg>; }
...
export default function ShopPage() {
  ...
  useEffect(() => {
    const params = new URLSearchParams(window.location.search); const requested = params.get("category"); const requestedType = params.get("type");
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (Array.isArray(catalogData?.categories)) setCategories(catalogData.categories);
      if (Array.isArray(catalogData?.subcategories)) setSubcategories(catalogData.subcategories);
    }).catch(() => {});
  }, []);

  const selectedCategory = categories.find((item) => item.name === category || item.slug === category);
  const selectedSubcategories = selectedCategory ? subcategories.filter((item) => item.category_id === selectedCategory.id) : [];
  const visibleCategories = categories.filter((item) => type === "All" || item.product_type === type).slice().sort((a, b) => a.sort_order - b.sort_order || a.name.localeCompare(b.name));

  const filtered = useMemo(() => {
    const term = query.trim().toLowerCase();
    const categoryNames = new Set(visibleCategories.map((item) => item.name));
    return [...products.filter((product) => {
      const productTypeMatch = type === "All" || product.product_type === type || (type === "Plants" && !product.product_type);
      const categoryMatch = category === "All" || product.category === category || product.category === selectedCategory?.name || product.category === selectedCategory?.slug || categoryNames.has(product.category);
      const subMatch = subcategory === "All" || product.subcategory === subcategory || product.subcategory === selectedSubcategories.find((item) => item.slug === subcategory || item.name === subcategory)?.name || product.subcategory === selectedSubcategories.find((item) => item.slug === subcategory || item.name === subcategory)?.slug;
      ...
      return productTypeMatch && categoryMatch && subMatch && levelMatch && (!term || text.includes(term)) && product.active;
  ...
  }, [category, level, products, query, sort, subcategory, selectedCategory?.name, selectedCategory?.slug, selectedSubcategories, type, visibleCategories]);
  ...
  const handleQuickAdd = (product: Product) => { if (product.stock < 1) return; addItem({ id: product.slug, name: product.name, price: Number(product.price), tone: product.tone, size: product.size, category: product.category, image_url: product.image_url || product.image_urls?.[0] || realPlantImages[product.slug] || null }, 1); setQuickAdded((items) => items.includes(product.slug) ? items : [...items, product.slug]); window.setTimeout(() => setQuickAdded((items) => items.filter((slug) => slug !== product.slug)), 1800); };
  ...
  return <main className="min-h-screen bg-[#f4f5e9] text-[#101510]">
    ...
    <section className="mx-auto max-w-7xl px-5 pb-8 sm:px-8"><div className="rounded-[30px] border border-black/10 bg-white/38 p-4 shadow-[0_12px_40px_rgba(32,45,32,.04)] sm:p-5"><div className="flex flex-col gap-3"><div className="flex flex-wrap gap-2"><Pill active={type === "All"} onClick={() => { setType("All"); chooseCategory("All"); }}>Everything</Pill><Pill active={type === "Plants"} onClick={() => { setType("Plants"); chooseCategory("All"); }}>Plants</Pill><Pill active={type === "Gardening Supplies"} onClick={() => { setType("Gardening Supplies"); chooseCategory("All"); }}>Gardening Supplies</Pill></div><div className="relative"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search plants, pots, soil, tools…" className="h-12 w-full rounded-full border border-black/10 bg-[#f4f5e9] px-5 text-sm outline-none transition placeholder:text-black/35 focus:border-black/25" /><span className="pointer-events-none absolute right-5 top-1/2 -translate-y-1/2 text-black/35">⌕</span></div><div className="flex gap-2 overflow-x-auto pb-1">{[<Pill key="all" active={category === "All"} onClick={() => chooseCategory("All")}>All categories</Pill>, ...visibleCategories.map((item) => <Pill key={item.id} active={category === item.name || category === item.slug} onClick={() => chooseCategory(item.name)}>{item.name}</Pill>)]}</div><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">{selectedCategory && selectedSubcategories.length > 0 ? <select value={subcategory} onChange={(event) => setSubcategory(event.target.value)} className="h-11 rounded-full border border-black/10 bg-[#f4f5e9] px-4 text-sm font-medium outline-none lg:min-w-[260px]"><option value="All">All in {selectedCategory.name}</option>{selectedSubcategories.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}</select> : <div className="text-xs text-black/45">Select a category to browse its subcategories.</div>}<div className="flex flex-wrap gap-2"><Pill active={level === "All"} onClick={() => setLevel("All")}>All care</Pill><Pill active={level === "Easy"} onClick={() => setLevel("Easy")}>Easy</Pill><Pill active={level === "Medium"} onClick={() => setLevel("Medium")}>Medium</Pill><select value={sort} onChange={(event) => setSort(event.target.value)} className="h-11 rounded-full border border-black/10 bg-[#f4f5e9] px-4 text-sm font-medium outline-none"><option value="featured">Featured</option><option value="low">Price: low to high</option><option value="high">Price: high to low</option></select></div></div></div></div></section>
