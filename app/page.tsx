...
const fallbackCollections: LinkItem[] = [
  { eyebrow: "01 / INDOOR", title: "Indoor & Decorative Greens", note: "Lush statement plants, tabletop greens and easy-care favourites.", href: "/shop?category=indoor-decorative-greens", slug: "indoor-decorative-greens", image: "https://images.unsplash.com/photo-1497250681960-ef046c08a56e?auto=format&fit=crop&w=1000&q=90" },
  { eyebrow: "02 / OUTDOOR", title: "Outdoor & Landscape Plants", note: "Flowering shrubs, hedges and larger plants for open-air spaces.", href: "/shop?category=outdoor-landscape-plants", slug: "outdoor-landscape-plants", image: "https://images.unsplash.com/photo-1501004318641-b39e6451bec6?auto=format&fit=crop&w=1000&q=90" },
  { eyebrow: "03 / PLANTERS", title: "Pots & Planters", note: "Plastic, terracotta, ceramic and fabric grow bags for every corner.", href: "/shop?category=pots-planters", slug: "pots-planters", image: "https://images.unsplash.com/photo-1610701596007-11502861dcfa?auto=format&fit=crop&w=1000&q=90" },
  { eyebrow: "04 / SOIL", title: "Soil & Growing Media", note: "Potting mix, cocopeat, red soil and compost for healthier roots.", href: "/shop?category=soil-growing-media", slug: "soil-growing-media", image: "https://images.unsplash.com/photo-1589923188900-85dae523342b?auto=format&fit=crop&w=1000&q=90" },
  { eyebrow: "05 / NUTRIENTS", title: "Fertilizers & Nutrients", note: "Organic manure, vermicompost and plant food to help growth thrive.", href: "/shop?category=fertilizers-nutrients", slug: "fertilizers-nutrients", image: "https://images.unsplash.com/photo-1585320806297-9794b3e4eeae?auto=format&fit=crop&w=1000&q=90" },
  { eyebrow: "06 / TOOLS", title: "Tools & Equipment", note: "Watering cans, pruners, trowels, spray bottles, stakes and trellises.", href: "/shop?category=tools-equipment", slug: "tools-equipment", image: "https://images.unsplash.com/photo-1416879595882-3373a0480b5b?auto=format&fit=crop&w=1000&q=90" },
];

const fallbackFooterShop: LinkItem[] = [
  { label: "Indoor plants", href: "/shop" }, { label: "Succulents", href: "/shop" }, { label: "Planters", href: "/shop" }, { label: "Plant care", href: "/pages/plant-care" },
];
const fallbackFooterAbout: LinkItem[] = [
  { label: "Our story", href: "/#story" }, { label: "Care journal", href: "/pages/plant-care" }, { label: "Account", href: "/account" }, { label: "Contact", href: "/#newsletter" },
];
...
export default function Home() {
  ...
  const cmsCollections = Array.isArray(content("collections").items) ? content("collections").items : [];
  const collections = cmsCollections.length >= 6 ? cmsCollections : fallbackCollections;
  ...
  const marqueeItems = Array.isArray(content("marquee").items) && content("marquee").items.length ? content("marquee").items.map((item: unknown) => String(item || "").trim()).filter(Boolean) : String(content("marquee").text || "PLANT MORE JOY").split("·").map((item: string) => item.trim()).filter(Boolean);
  ...
  function handleAdd(product: CmsProduct) {
  ...
  return <main className="verdant-site">
    ...
    {visible("footer") && <footer className="footer" id="account"><div className="footer-brand"><Link className="brand light" href="/"><span className="brand-mark"><LeafMark /></span><span>VERDANT</span></Link><p>{footer.description || "Thoughtful plants and beautiful objects for a greener everyday."}</p></div><div className="footer-col"><span>SHOP</span>{footerShopLinks.map((item: LinkItem, index: number) => <Link href={item.href || "/shop"} key={`${item.label}-${index}`}>{item.label}</Link>)}</div><div className="footer-about-with-circular"><div className="footer-col"><span>ABOUT</span>{footerAboutLinks.map((item: LinkItem, index: number) => <Link href={item.href || "#story"} key={`${item.label}-${index}`}>{item.label}</Link>)}</div><div className="footer-circular-wrap"><CircularText text="GREENS*PLANTS*GARDENING*" onHover="speedUp" spinDuration={20} className="verdant-footer-circular" /></div></div><div className="footer-end"><span>{footer.copyright || "© 2026 VERDANT"}</span><span>{footer.tagline || "MADE FOR SLOW GROWERS"}</span></div></footer>}
  </main>;
}
