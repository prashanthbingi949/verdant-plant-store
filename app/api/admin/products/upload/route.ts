...
export async function POST(request: Request) {
  ...
  try {
    ...
    const productSlug = String(formData.get("slug") || "product")
      .toLowerCase()
      .replace(/[^a-z0-9-]+/g, "-")
      .replace(/(^-|-$)/g, "") || "product";
    ...
    const existing = await supabaseSelect(
      "products",
      `select=image_url,image_urls&slug=eq.${encodeURIComponent(productSlug)}&limit=1`,
    );

    if (!existing.configured || !existing.response?.ok) {
      return NextResponse.json({ error: "Image uploaded, but the product record could not be read." }, { status: 502 });
    }

    if (!Array.isArray(existing.data) || !existing.data[0]) {
      return NextResponse.json({ error: "Image uploaded, but no matching product was found." }, { status: 404 });
    }

    const current = existing.data[0] as { image_url?: string | null; image_urls?: unknown };
    const currentUrls = Array.isArray(current.image_urls)
      ? current.image_urls.filter((value): value is string => typeof value === "string")
      : [];
    const nextUrls = [...new Set([...currentUrls, url])].slice(0, 8);

    const updated = await supabaseUpdate("products", `slug=eq.${encodeURIComponent(productSlug)}`, {
      image_url: current.image_url || url,
      image_urls: nextUrls,
      updated_at: new Date().toISOString(),
    });

    if (!updated.configured || !updated.response?.ok) {
      return NextResponse.json({ error: "Image uploaded, but it could not be attached to the product." }, { status: 502 });
    }
