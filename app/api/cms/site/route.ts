...
export async function GET() {
  ...
  if (settingsResult.configured && settingsResult.response?.ok && Array.isArray(settingsResult.data)) {
    for (const row of settingsResult.data as { site_key: string; value: Record<string, unknown> }[]) settings[row.site_key] = row.value || {};
  }
  const navigation = navResult.configured && navResult.response?.ok && Array.isArray(navResult.data) && navResult.data.length ? navResult.data : defaultNavigation;
