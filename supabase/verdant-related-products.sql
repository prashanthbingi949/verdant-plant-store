...
resolved as (
  select e.*, c.id as category_id, c.name as category_name, c.product_type,
         sc.name as subcategory_name, sc.sort_order as sub_sort
  from expanded e
...
select
  ...
  true,
  false,
  (r.category_id::text::bit(32)::int % 1000) + (r.sub_sort * 10) + r.ordinal,
  '',
  r.asset_path,
...
-- Bring the original demo products into the appropriate subcategories.
